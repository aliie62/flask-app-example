    ...
    def post(self):
        try:
            ...
            try:
                ...
                if existing_store:
                    return {
                        "message": f"Store with name <{existing_store.name}> already exists."
                    }, 400
                else:
                    store.save_to_db()
        ...
        try:
            ...
            try:
                ...
                if item and store:
                    ...
                    store.save_to_db()
                    return {
                        "message": f"Item <{item.name}> was allocated to store <{name}> successfully."
                    }, 200
                else:
                    return {
                        "message": f"Item <{item.name}> or/and store <{name}> were not found in our database."
                    }, 400
            except:
                return {
                    "message": "An error occurred in allocating item to the store."
                }, 500
